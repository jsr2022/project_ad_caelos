        ...
        if data_label is not None:
            ...
            if isinstance(data_label, str):
                try:
                    ...
                    errorMsg = f"Data label '{data_label}' not found in stored data."
            ...
            # Multiple labels (list)
                ...
                for label in data_label:
                    if not isinstance(label, str):
                        errorMsg = f"All data labels must be strings. Got {type(label).__name__}: '{label}'."
                        raise TypeError(errorMsg)
                    try:
                        result[label] = self.__data_storage[label]
                    except KeyError as e:
                        errorMsg = f"Data label '{label}' not found in stored data."
                        raise KeyError(errorMsg) from e
                return result

            else:
                errorMsg = f"data_label must be a string or a list of strings. Got {type(data_label).__name__}."
                raise TypeError(errorMsg)
